...
\noindent PsyGeNET is a resource for the exploratory analysis of psychiatric diseases and their associated genes. It is focused on three psychiatric disorders: major depression, alcohol use disorder and cocaine use disorder. PsyGeNET allows the exploration of the molecular basis of psychiatric disorders by providing a comprehensive set of genes associated to each disease. Moreover, it allows the analysis of the molecular mechanisms underlying psychiatric disease comorbidities. PsyGeNET database is the result of the integration of information from DisGeNET \cite{REF3} and data extracted from the literature by text mining \cite{REF4}, followed by domain experts curation. The current version of PsyGeNET (version?) contains 2,642 associations, between 1,271 genes and 37 psychiatric disease concepts.\\
...
Name             & Description                                                \\
    \hline
\texttt{MODELS}  & Comparative Toxigenomics Database, data from mouse and rat \\
\texttt{GAD}     & Genetic Association Database                               \\
\texttt{CTD}     & Comparative Toxigenomics Database, data from human         \\
\texttt{PsyCUR}  & Psychiatric disorders Gene association manually curated    \\
\texttt{CURATED} & Human, manually curated databases: PsyCUR, CTD             \\
...
\begin{figure}[H]
\begin{center}
<<plot_psychiatric1, fig=TRUE, widtgh=10>>=
...
<<heatmap_disease_m, fig=TRUE,width=8, height=8 >>=
plot( m1, type="heatmapGenes", cutOff=0.5 )
@
...
\noindent In addition, the results of a query using a list of genes can be visualized using a Venn diagram. Here we show the overlap between the diseases that result from the query in terms of the number of shared genes. \texttt{psygenet2r} package provides two types of Venn diagrams. For the classical Venn diagram, the argument \texttt{type} is \texttt{"venn"}. For the alternative Venn diagram, the argument \texttt{type} must be set to \texttt{"vennA"}.\\

<<plot_vennDiagram1, fig=TRUE, height=8, width=8>>=
plot( m1, type = "venn" )
@

<<plot_vennDiagram2, fig=TRUE, height=8, width=8>>=
plot( m1, type = "vennA" )
@

\noindent Previous figures show both results, the classical venn diagram and the alternative one. The alternative Venn diagram is represented by a triangle. In the center the genes that are common for the three psychiatric disorders are represented (in this case 10 genes), while in the vertex the genes that are exclusive to each disorder are represented. As it can be seen, there are 4 genes that are unique to Major Depression, whereas there are no genes that are unique to either Alcohol Use disorder or Cocaine use disorder. However there are 5 genes that are in common for alcohol use disorder and major depression and one that is in common for cocaine use disorder and major depression.\\


\noindent An alternative graphical representation is a psychiatric disorder heatmap. Two different options are able here, one taking into account the mean score and the other one related to the percentage of diseases to which a gene appear associated.\\

\noindent The one related to the score, allows to see the score mean that a gene present with the different diseases present in the same psychiatric disorder.The resultant values are represented in a heatmap according to a orange color scale, being the darkest ones  those with the highest score mean.\\

\noindent In order to obtain this graphic, \texttt{type} must be set to \texttt{"heatmapScore"}. The resulting heatmap shows which are the genes that are higher or lower score to each one of the psychiatric disorders.\\ 
...
<<plot_psychiatric_heamap, fig = TRUE>>=
...
geneSentences <- extractSentences( input = sss,
...
d2 <- psygenetDisease( disease = "Single Major Depressive Episode", 
                       database = "ALL",
                       score    = c('>', 0.5 ) )
d2
@
...
<<plot_visualizing_single_disease_search, fig=TRUE>>=
plot ( d1 )
@
...
<<search_diseases_3>>=
tm <- psygenetDisease( disease  = c( "Bipolar Disorder","umls:C1269683" ), 
                       database = "ALL" )
tm
@
...
<<heatmap_disease_tm, fig=TRUE>>=
plot( tm, type = "heatmap", score = 0.7 )
@
...
<<barplot_visualizing_single_disease_search, fig=TRUE, width=5, height=3>>=
plot ( d1, name = "Single Major Depressive Episode", type = "barplot" )
@
...
<<barplot_visualizing_single_gene_search, fig=TRUE, width=5, height=3>>=
...
ji1 <- jaccardEstimation(genes_interest, database = "CURATED")
@
...
<<ji_2>>=
disease_interest <- 
  c("Delirium", "Bipolar Depression", "Recurrent Depression", "Mood Disorders")
ji2 <- jaccardEstimation(genes_interest, disease_interest, database = "CURATED")
@
  \item With a list of diseases of interest, the function will calculate the Jaccard Index between themselves:
<<ji_3>>=
ji3 <- jaccardEstimation(disease_interest, database = "CURATED")
@
\end{enumerate}
%' 
...
<<ji1_plot, fig=TRUE, width=5, height=3>>=
plot(ji1, cutOff = 0.1)
@
...
<<ji2_plot, fig=TRUE, width=5, height=3>>=
plot(ji2)
@
...
<<ji3_plot, fig=TRUE, width=5, height=3>>=
plot(ji3)
@
