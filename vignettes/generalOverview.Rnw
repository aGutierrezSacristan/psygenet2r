...
\begin{table}[htp]
    ...
    \label{tab:psygenet_visualization options}
    \resizebox{\textwidth}{!}{%
        \begin{tabular}{|l|l|l|l|}
            \hline
            \textbf{Input Object} & \textbf{\texttt{psygenet2r} function} & \textbf{Argument Type} & \textbf{Output Generated}\\
            \hline
     \multirow{9}{*}{DataGeNET.Psy}  &  \multirow{4}{*}{geneAttrPlot} 	& category & genes asociated with each DC \\
     ...
     & &   index    & barplot showing the type of association\\
         \cline{2-4}
     &  \multirow{5}{*}{plot} 	& individual disease & GDA network (default type) \\
