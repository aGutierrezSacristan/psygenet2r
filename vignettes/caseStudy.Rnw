...
\begin{enumerate}
\item Are the genes associated to psychiatric disorders according to PysGeNET?
...
\item Is bipolar disorder similar to other psychiatric disorders based on shared genes?
\end{enumerate}
...
\noindent In order to know how many of the genes of interest are present in PsyGeNET, \texttt{psygenetGeneList} function is used. This function requires as input the genes' vector and the selected database. For this analysis \texttt{"ALL"} database are selected.\\ 
...
\noindent The output is a \texttt{DataGeNET.Psy} object. It contains all the information about the different diseases associated with the genes of interest retrieved from PsyGeNET. By looking at the \texttt{DataGeNET.Psy} object, it can be observed that, according to PsyGeNET and by querying in \texttt{ALL} databases, 16 of the initial genes are found in PsyGeNET. These genes appear associated with 15 different disorders, involving a total of 48 gene-disease associations (GDAs).\\

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsection{Which diseases are associated to these genes according to PsyGeNET database?}

\noindent In order to visualize the 48 GDAs between the 16 genes found in PsyGeNET and the 15 different disorders, \texttt{psygenet2r} provides several options. One of them is the GDA network, which can be obtained by applying the \texttt{plot} function to the \texttt{DataGeNET.Psy} object (\texttt{m1}), obtained from \texttt{psygenetGene} function (section 3.1). In the GDA network, green nodes represent diseases and orange nodes represent genes.\\

\begin{figure}[H]
\begin{center}
<<plot_disease_m, fig.height=8, fig.width=8>>=
plot( m1 )
@
...
\noindent As shown in Figure \ref{gene-disease}, most of the genes (10) are associated to bipolar disorder (umls:C0005586), in agreement with McCarthy et al. \cite{McCarthy}, but some of these genes are also associated to other disorders related to alcohol UD, depression and schizophrenia.\\

\noindent In PsyGeNET it is important to keep track of both “positive” and the “negative” findings, and let the user make their own judgements based on the available evidence. We can visualize the association type (Association, No Association,Both) using heat-maps.\\

\noindent In the example shown in the next figure (Figure \ref{gene-psy}), the gene-disease associations related to alcohol UD and depresison are all of type "Association". On the other hand, for bipolar disorder and schizophrenia there are some GDAs that have both types of associations. Finally, in the schizophrenia category it can be seen that there is one GDA with "no association" type.\\

\noindent The barplot can be obtained using the \texttt{geneAttrPlot} function and by setting the \texttt{type} argument to \texttt{"index"}.\\

\begin{figure}[H]
\begin{center}
<<plot_psychiatric_m1Score, fig.height=8, fig.width=8>>=
geneAttrPlot( m1, type = "index" )
@
\caption{Association type barplot according to psychiatric category}
\label{gene-psy}
\end{center}
\end{figure}
...
\noindent \texttt{psygenet2r} package can be used to analyze gene attributes such as the panther class. The \textsl{PANTHER Protein Class Ontology} includes commonly used classes of protein functions. The Panther class can be analyzed using the function \texttt{pantherGraphic}.\\ 
...
\begin{figure}[H]
\begin{center}
<<ggplot_panther, fig.height=8, fig.width=8, message=FALSE, warning=FALSE>>=
pantherGraphic( genesOfInterest, "ALL")
@
\caption{Panther class analysis of the genes of interest.}
\label{panther}
\end{center}
\end{figure}

\noindent The bar-plot shown in Figure \ref{panther} is obtained from the gene-list of interest. All the genes in the list that are associated with the psychiatric disease class of alcohol use disorders are signaling molecules. On the other hand, it can be observed that those genes associated with bipolar disorder, depression and schizophrenia are in a variety of Panther classes.\\

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsection{What is the level of evidence for each GDA?}

\noindent In PsyGeNET, each GDA is ranked with the PsyGeNET evidence index, that reflects the association type for each GDA. We can use \texttt{psygenet2r} package to visualize the level of evidence in a heatmap.\\

\noindent In order to obtain the heatmap, the \texttt{plot} function can be applied to the \texttt{DataGeNET.Psy} object (\texttt{m1}). The argument type must to be set to \texttt{"heatmapGenes"}.\\

\noindent As a result a heat-map is obtained with genes in the X axis and disorders in the Y axis. Heatmap cells will be coloured in green, yellow or red according to the evidence index value. Green color represents those GDAs where all the evidences reviewed by the experts support the existence of an association between the gene and the disease (EI = 1); it will be yellow when there is contradictory evidence for the GDA (some publications support the association while others publications do not support it, 1 > EI > 0), and it will be red when all the evidences reviewed by the experts report that there is no association between the gene and the disease (EI = 0).\\  

\begin{landscape}
\begin{figure}[H]
\begin{center}
<<heatmap_disease_m, fig.width=10, fig.height=4.5, fig.align='center'>>=
plot( m1, type="heatmapGenes")
@
\caption{Gene-Disease Association Heatmap}
\label{gene-disease-2}
\end{center}
\end{figure}
\end{landscape}

\noindent Figure \ref{gene-disease-2} shows that from the total 48 GDAs, only 4 of them have an evidence index different from 1. For example, all the publications supporting C15orf53-schizophrenia association report that there is no association; for the other 3 GDAs (C15orf53-bipolar disorder, DLG2-schizophrenia and SYNE1-bipolar disorder)contradictory evidences have been found.\\

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsection{For the disorder of interest, how many publications support each gene-disease associations?}

\noindent In addition to the PsyGeNET evidence index, we can also inspect the number of publications that support a GDA. \texttt{psygenet2r} allows the visualization of this information in a bar-plot, using the \texttt{plot} function with the \texttt{disorder} argument to indicate the disease of interest, and the \texttt{type} argument set to \texttt{"barplot"}. Figure \ref{pubmed-2} shows an example, with the genes in the X-axis and the number of PMIDs in the y-axis.\\

\begin{figure}[H]
\begin{center}
<<pmids_barplot, fig.height=8, fig.width=8>>=
plot( m1, name="bipolar disorder", type="barplot")
@
\caption{Publications that report each gene association with bipolar disorder}
\label{pubmed-2}
\end{center}
\end{figure}

\noindent The results show that the CACNAC1 gene is associated with bipolar disorder in more than 50 publications, followed by ANK3 gene with around 40 publications. The rest of genes have been associated with bipolar disorder in less than 10 publications.\\
...
\noindent The result shows the number of iterations used to calculate the p-value and the type of input, in this case a list of genes and a disease. The Jaccard Index and the p-value can be extracted using the function \texttt{extract}:

<<jaccard_2>>=
extract( xx )
@

\noindent Now we have seen the Jaccard Index and its p-value, between our genes of interest and bipolar disorder (\textit{JI}: 0.018; \textit{pval}: 0). The function \texttt{jaccardEstimation} also allows to calculate the Jaccard Index of our input, the list of genes, and all the diseases in PsyGeNET.
...
\noindent The result from the Jaccard Index estimation can be visualized using the function \texttt{plot}. The result is a bar-plot where the p-value of each comparison between our genes and PsyGeNET's disease is shown. A \texttt{cutOff} argument can be added in order to visualize only those diseases with an statistically significant p-value:

\begin{figure}[H]
\begin{center}
<<jaccard_4, fig.height=5>>=
plot( xx )
@
\end{center}
...
\noindent The similarity between diseases according to shared genes can be visualized using a barplot. \texttt{psygenet2r} package allows to visualize how many of our genes of interest are associated to each psychiatric disorder present in PsyGeNET and how many of them are exclusively associated to a particular psychiatric disorder.\\

\noindent In order see the similarity between our genes of interest and PsyGeNET's psychiatric disorder, the \texttt{type} argument of \texttt{geneAttrPlot} function is set to \texttt{category}. This will allow to see how many of the 16 genes, that appear in PsyGeNET, are associated with each psychiatric disorder.

\begin{figure}[H]
\begin{center}
<<plot_bpGenes, fig.height=8, fig.width=8>>=
geneAttrPlot( m1, type = "category" )
@
\caption{Barplot: Genes associated to each of the psychiatric disorders}
\label{bpGenes}
\end{center}
\end{figure}

\noindent Figure \ref{bpGenes} clearly shows that none of our genes of interest are associated to the three disorders. There are 2 exclusively associated to bipolar disorder, 2 in the case of depression and 3 in schizophrenia. The rest of the genes appears in more than one psychiatric disorder.\\

\noindent In order see which are the genes that are associated only to one psychiatric category, the \texttt{type} argument of \texttt{geneAttrPlot} function can be set to \texttt{gene}. This will allow to see how many of the CUIs and categories are associated to each one of the 16 genes. 

\begin{figure}[H]
\begin{center}
<<plot_bpDis, fig.height=8, fig.width=8>>=
geneAttrPlot( m1, type = "gene" )
@
\caption{Barplot: CUIs and psychiatric categories associated to each gene}
\label{bpDis}
\end{center}
\end{figure}

\noindent Figure \ref{bpDis} clearly shows that these genes are: ADCY2, ATXN1, CACNA1C, HHAT, MAD1L1, NFIX and SH3PXD2A. Yellow bar for these genes show that they are only associated with one psychiatric disorder.\\
