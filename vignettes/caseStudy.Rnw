...
\noindent We can also visualize the association type (Association, No Association,Both) using heat-maps.\\
...
\noindent In PsyGeNET it is important to keep track of both “positive” and the “negative” findings, and let the user make their own judgements based on the available evidence.\\
...
\begin{figure}[H]
\begin{center}
<<heatmap_disease_m, fig=TRUE, fig.width=10, fig.height=5.8, fig.align='center'>>=
...
@
\caption{PubMed Ids that report each gene association with bipolar disorder (manic depressive illness).}
\label{pubmed-2}
\end{center}
\end{figure}
