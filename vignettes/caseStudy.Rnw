...
\begin{figure}[H]
\begin{center}
<<ggplot_panther, fig=TRUE, height=8, width=8, message=FALSE, warning=FALSE>>=
...
\begin{figure}[H]
\begin{center}
<<heatmap_disease_m, fig=TRUE, fig.width=10, fig.height=4.5, fig.align='center'>>=
...
\begin{figure}[H]
\begin{center}
<<plot_bpGenes, fig=TRUE, height=8, width=8>>=
...
\noindex In order see which are the genes that are associated only to one psychiatric category, the \texttt{type} argument of \texttt{geneAttrPlot} function can be set to \texttt{gene}. This will allow to see how many of the CUIs and categories are associated to each one of the 16 genes. 

\begin{figure}[H]
\begin{center}
<<plot_bpDis, fig=TRUE, height=8, width=8>>=
